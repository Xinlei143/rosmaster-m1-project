...
/usr/bin/colcon build \
  --packages-select imperative_m1_description imperative_navigation \
  --symlink-install
```
...
```text
src/imperative_m1_description/urdf/yahboomcar_M1.urdf.xacro
    原厂 M1 机器人描述
src/imperative_m1_description/urdf/yahboomcar_M1_gazebo.urdf.xacro
    Gazebo 底盘插件、里程计和 T-MINI PLUS 雷达扩展
src/imperative_m1_description/meshes/M1Mecanum/
    M1 外观 STL 网格
```
...
`imperative_m1_straight_2m.launch.py` 使用与 Gazebo 相同的局部轨迹规划、激光聚类和动态障碍物预测；它在收到第一帧有效 `/odom` 时锁定“初始车头前方 2 m”的目标。遇到障碍物可以绕行，因此实际路径不一定严格为直线。控制器默认不允许运动，且必须先在另一个终端保持 watchdog 运行。
...
确认场地、`/scan`、`/odom`、TF 与 watchdog 正常后，显式启用：

```bash
PYTHONPATH="/home/lin24311/car_ws/.venv-ros2-ml/lib/python3.10/site-packages:$PYTHONPATH" \\
ros2 launch imperative_navigation imperative_m1_straight_2m.launch.py enabled:=true
```

默认最大速度为 `0.12 m/s`、最大加速度为 `0.20 m/s²`；到达目标容差 `0.08 m` 后控制器会保持发布零速度。
