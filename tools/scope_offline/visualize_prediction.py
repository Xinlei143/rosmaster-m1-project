...
def render(dataset_path, predictions_path, output_path, rank=None):
    ...
    selected_rank = representative_rank(len(indices)) if rank is None else int(rank)
    ...
    figure.suptitle(
        "M1 SCOPE zero-shot diagnostic: evaluation rank %d, dataset window %d\n"
        "Black = occupied endpoint for binary panels; prediction is probability"
