...
- **离线零样本评估**：冻结的官方预训练 SCOPE 在动态障碍物 ROI 的 `0.5 s` 预测上优于 `copy-last`，但在 `1.0 s` 上低于该基线。
- **ROS 2 在线观测**：冻结模型已封装为 `m1_scope_predictor`，完成与 Gazebo + Nav2 的联合启动、运行时一致性与诊断链路检查。

两条证据线共享同一套 M1 栅格预处理，但回答的问题不同：前者量化给定 bag 上的预测重叠，后者验证在线数据链路。在线节点不发布 `/cmd_vel`，不修改 Nav2 costmap，也不参与 MPPI 或 Imperative 控制；因此本记录不构成闭环导航收益、实时保证、实机部署能力或安全性证明。

下面依次记录共同数据协议、离线评估、在线观测、可视化与可追溯产物。

## 1. 离线零样本评估的目的与边界

本记录验证：不使用 M1 数据重新训练，直接加载官方预训练 SCOPE，能否预测 M1 Gazebo 场景中动态障碍物在未来 0.5 s 和 1.0 s 的占据位置。

本节是离线 OGM 预测评估：不将 SCOPE 接入 Nav2，不发布控制指令，也不修改 Imperative 控制器。
...
`m1_scope_predictor` 在完成上述冻结离线评估后新增。它订阅 `/scan` 和 `/odom`，复用离线导出器的十帧、64×64 历史栅格预处理，以最新任务优先的方式异步执行 SCOPE 推理，并发布当前端点栅格、0.5 s 预测概率栅格、不确定性栅格和诊断信息。Nav2 必须显式设定 `scope_enabled:=true` 才会启动该节点，默认值仍为 `false`。

严格的 Python 3.7 / PyTorch 1.7.1 离线复现实验仍是参考基线。在线节点在独立的 Python 3.10 虚拟环境中运行 PyTorch 2.5.1+cu124，并在启动时以 SHA-256 校验外部 checkpoint 与随包保留的官方 `model.py`、`convlstm.py`。相同 0.5 s、32-sample 协议下，对 100 个离线窗口的新旧运行时比较得到：平均预测概率 MAE 为 `0.000449`；总体 F1/IoU 绝对差为 `0.000956/0.001074`；动态 ROI F1/IoU 绝对差为 `0.000905/0.000641`。这些数值只构成运行时一致性门禁，不是模型精度或导航性能指标。
...
`/scope/diagnostics` 的在线评估器可输出预测与未来激光观测的端点栅格 MAE、IoU 和 F1。它使用 Gazebo `/ground_truth/odom` 对齐未来扫描，只用于时间/坐标链路自检。原始 bag 已包含 `/clock`，回放时不得再传入 `ros2 bag play --clock`，否则两个时钟源会使预测年龄诊断失真。

启动参数、话题语义与坐标系定义见 [SCOPE 在线节点说明](../scope_online.md)。
